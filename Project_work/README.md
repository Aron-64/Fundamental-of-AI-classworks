...
### 2025.12
1. Web交互系统尚未完成：项目中包含了一个基于Flask的web应用（位于webapp文件夹），旨在提供交互式的旅行规划界面。用户可以通过网页输入出发城市、目的地、旅行天数、人数、风格和预算等参数，系统将调用后端Agent生成旅行行程。目前该系统已实现基本的前端界面和后端API，但存在以下问题：
   - 后端集成不完整：Agent调用可能存在错误或未完全适配web环境。
   - 前端功能有限：仅支持基本参数输入，缺乏结果展示优化和错误处理。
   - 部署和测试不足：未进行充分的集成测试和部署验证。
   - 安全性考虑：未添加用户输入验证和安全防护措施。

   **项目修改和增加说明**：
   - **新增webapp文件夹**：包含Flask应用(app.py)和HTML模板(index.html)，为用户提供web界面。
   - **Agent集成尝试**：在app.py中尝试集成现有的Agent和LLM模块，但可能需要进一步调试。
   - **前端界面设计**：提供简单的表单输入和结果显示区域，支持实时生成行程。
   - **未来改进方向**：建议完善Agent调用逻辑，增强前端交互性，添加用户会话管理，并进行全面测试以确保系统稳定性。

2. 添加新的线上API调用方式：除了本地模型和vLLM推理，新增了对Qwen API的直接调用支持。通过DashScope平台，用户可以无需本地GPU资源，直接使用Qwen模型进行推理。
   - **新增QwenApiLLM类**：在chinatravel/agent/llms.py中实现，支持qwen-max-latest和qwen-plus-latest等模型。
   - **集成到load_model.py**：添加"qwen_api"选项，支持在实验脚本中指定--llm qwen_api。
   - **优势**：降低本地计算资源需求，简化部署，支持快速原型开发。
   - **使用示例**：python run_exp.py --splits human --agent LLMNeSy --llm qwen_api
...
We support the deepseek (offical API from deepseek), gpt-4o (chatgpt-4o-latest), glm4-plus, qwen_api (DashScope API for Qwen models), and local inferences with Qwen (Qwen3-8B), llama, mistral (Mistral-7B-Instruct-v0.3), etc.
...
python run_exp.py --splits human --agent LLMNeSy --llm qwen_api 
...
```
## Acknowledgement
We thank the authors of ChinaTravel for releasing their codebase.